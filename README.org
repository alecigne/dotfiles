...
Guix-managed packages. Guix will install the packages defined in [[file:packages/.packages/guix-manifest.scm][my
manifest file]] by running this command:

#+begin_src sh
  guix package -m ~/.packages/guix-manifest.scm
#+end_src
